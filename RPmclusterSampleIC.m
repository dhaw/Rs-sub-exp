function f=RPmclusterSampleIC(G,mmax,sampleNumber)%Or input v
...
for m=2:mmax
    for i=1:lv
        %m-neighbours of v(i):
        vi=v(i);
        vx=david.Cv{i};%m-1 OR m (if already xcalculated in loop) nbrs of v(i)
        imvw2i=imvw2(i);
        if imvw2i<m%i.e.==m-1
        ...
        for j=1:lvm%parfor
            ...
            if ismember(vmj,imvw1)%If have already calculated some nbhds
                ...
                thisFar=imvw2(index);
                vmjSoFar=david.Cv{index};
                if thisFar<m
                    mNj=mNbr(G,vmjSoFar,m-thisFar,vmj);
                    imvw2(index)=m;
                    david.Cv{index}=mNj;
                else
                    mNj=david.Cv{index}; %mNjOnly=Cw{index};
                end
            else
